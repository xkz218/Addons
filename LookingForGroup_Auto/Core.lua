		...
		hardware_func(SIGN_UP,function()
			...
			C_Timer.After(5,function()
				...
				if status == "applied" then
					StaticPopupDialogs.LookingForGroup_HardwareAPIDialog.button3 = nil
...
function LookingForGroup_Auto.accepted(create,search,secure,name,value)
	...
	if IsInGroup() then
		...
		elseif search then
			...
			dialog.OnAlt = create
			if not secure and (profile.auto_find_a_group or profile.hardware) then
				dialog.text = SEARCH
				dialog.OnAccept = search
				StaticPopup_Show("LookingForGroup_HardwareAPIDialog")
			else
				search()
			end
...
function LookingForGroup_Auto.abandon(func)
	...
	if GetNumGroupMembers() == 0 then
		...
		local t = StaticPopupDialogs.LookingForGroup_HardwareAPIDialog
		t.text = PARTY_LEAVE
