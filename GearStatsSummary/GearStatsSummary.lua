...
function GearStatsSummary_Sum(inspecting, tipUnit)
	...
	for i=INVSLOT_FIRST_EQUIPPED, INVSLOT_LAST_EQUIPPED do --zhengruiw02
		...
		if (link) and i ~= INVSLOT_BODY and i ~= INVSLOT_TABARD then
			...
			if unitLevelDB and guid and unitLevelDB[guid] then
				iLevel = unitLevelDB[guid][i]
			end
			...
			if quality and (quality >=2 and quality <=7) then
				if quality == 6 and iLevel == 750 and (i == 16 or i == 17) then
					...
					if delay then
						C_Timer.After(0.1, function()
							iLevel = ScanItemTooltip(GetInventoryItemLink(unit, i))
						end)
					else
					--	iLevel = ScanItemTooltip(GetInventoryItemLink(unit, i == 16 and 17 or 16))
						if guid and unitLevelDB and unitLevelDB[guid] then
							iLevel = unitLevelDB[guid][16]
						else
							iLevel = ScanItemTooltip(GetInventoryItemLink(unit, i == 16 and 17 or 16))
						end
					end
				...
				sum["ITEMLEVEL"..quality] = (sum["ITEMLEVEL"..quality] or 0) + (iLevel or 0);
				sum["ItemLink"][i] = "["..(iLevel or '...').."]"..ElvUI[1]:RGBToHex(r,g,b).."["..itemName.."]|r";
			end
			...
			if quality and quality == 6 then --增加圣物显示
				local index = INVSLOT_LAST_EQUIPPED + 1;
				for rs = 1, 3 do
					local reliclink = select(2, GetItemGem(link, rs))
					if reliclink then
						local n, _, q, lv, _, _, subType, _ = GetItemInfo(reliclink); 
						local rr, gg, bb = GetItemQualityColor(q);
						sum["ItemLink"][index] = "["..lv.."]"..ElvUI[1]:RGBToHex(rr,gg,bb).."["..subType.."]["..n.."]|r";
						index = index + 1;
					end
				end
			end
