...
function LookingForGroup_SF:OnEnable()
	local profile = LookingForGroup.db.profile
	local api = profile.enable_sf and ChatFrame_AddMessageEventFilter or ChatFrame_RemoveMessageEventFilter
	...
	do
		local rapi = profile.spam_filter_channel_guild and api or ChatFrame_RemoveMessageEventFilter
		rapi("CHAT_MSG_GUILD",guild_filter)
	end
	do
		local rapi = profile.spam_filter_channel_instance and api or ChatFrame_RemoveMessageEventFilter
