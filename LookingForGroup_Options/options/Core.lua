...
LookingForGroup_Options:push("options",{
	...
	args =
	{
		...
		ms =
		{
			...
			set = function(info,val)
				if val then
					LookingForGroup.db.profile.enable_ms = true
				else
					LookingForGroup.db.profile.enable_ms = nil
				end
			end,
		},
		disable_blizzard =
				...
				gmotd =
				{
					...
					set = function(info,val)
						...
						if val then
							...
							QuickJoinToastButton:Hide()
							if status then
								LookingForGroup_Hook:Unhook("ChatFrame_DisplayGMOTD")
							end
						else
							LookingForGroup.db.profile.hook_gmotd = nil
							QuickJoinToastButton:Show()
							if status then
								LookingForGroup_Hook:RawHook("ChatFrame_DisplayGMOTD",function()end,true)
							end
						end
					end,
				}
