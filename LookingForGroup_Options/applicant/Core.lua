...
local function member_info(t,name, class, localizedClass, level, itemLevel, honorLevel, tank, healer, damage, assignedRole, relationship)
	...
	if LookingForGroup.lfgscores then
		local score_func = LookingForGroup.get_lfg_score_tooltip_info
		if score_func == nil then
			LoadAddOn("LookingForGroup_Score")
			score_func = LookingForGroup.get_lfg_score_tooltip_info
		end
		local score = score_func(name)
		if score then
			concat_tb_tooltip[#concat_tb_tooltip+1] = "\n"
			concat_tb_tooltip[#concat_tb_tooltip+1] = score
		end
	end
end
...
local function applicant_info(appid)
	...
	local data
	if not LookingForGroup.db.profile.complete then
		local summary
		summary,data = string_match(comment,'^(.*)%((^1^.+^^)%)$')
		if summary then
			comment = summary
		end
	end
	...
	if comment and comment ~= "" then
		...
		concat_tb_tooltip[#concat_tb_tooltip+1]='\n\n'
		concat_tb_tooltip[#concat_tb_tooltip+1]=comment
	end
	if data and LookingForGroup.db.profile.enable_ms then
		if LookingForGroup_MS == nil then
			LookingForGroup_MS = LookingForGroup.GetAddon("LookingForGroup_MS")
		end
		local status,info = pcall(LookingForGroup_MS.hap,data)
		if status then
			concat_tb_tooltip[#concat_tb_tooltip+1] = info
		end
	end
