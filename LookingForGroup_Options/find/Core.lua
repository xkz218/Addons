...
--[[local function get_preferences()
	local pre = LookingForGroup_Options.db.profile.a_group_preferred_filters
	if pre then
		return LE_LFG_LIST_FILTER_NOT_RECOMMENDED
...
local function get_search_result()
	...
	for i=1,#applications do
		...
		if select(2,GetApplicationInfo(number)) == "applied" then
			results_table[#results_table+1] = number
		end
	end
			...
			args =
			{
				...
				Group =
				{
					...
					set = function(info,val)
						...
						local name=C_LFGList_GetActivityGroupInfo(val)
						if name then
							profile.find_a_group_activities[1] = name
						else
							table.remove(profile.find_a_group_activities,1)
						end
						profile.a_group_activity = 0
						wipe(encounters_tb)
						wipe(profile.find_a_group_encounters)
					end,
				...
				activity_id =
				{
					...
					set = function(info,val)
						if val == "" then
							...
							local profile = LookingForGroup_Options.db.profile
							if val == 0 then
								profile.a_group_group = 0
								profile.a_group_activity = 0
								profile.find_a_group_activities[1] = nil
							else
								local longname,shortname,cat,grp = C_LFGList_GetActivityInfo(val)
								local v = longname or shortName
								if v and cat and grp then
									profile.a_group_category = cat
									profile.a_group_group = grp
									profile.a_group_activity = val
									profile.find_a_group_activities[1] = v
								end
							end
							wipe(encounters_tb)
							wipe(profile.find_a_group_encounters)
				...
				Activity =
				{
					...
					set = function(info,val)
						local profile = LookingForGroup_Options.db.profile
						profile.a_group_activity = val
						local longname,shortname= C_LFGList_GetActivityInfo(val)
						local v = longname or shortName
						if v then
							profile.find_a_group_activities[1] = v
						else
				...
				reset = 
				{
					...
					func = function()
						...
						local profile = LookingForGroup_Options.db.profile
						profile.a_group_legacy = nil
						profile.a_group_preferred_filters = nil
						wipe(encounters_tb)
						wipe(profile.find_a_group_encounters)
						wipe(activities_select_tb)
						wipe(filters_select_sup)
						wipe(profile.find_a_group_activities)
					end
				},
				recommanded =
				{
...
--[[				preferred =
				{
					...
					type = "toggle",
					get = function(info)
						local legacy = LookingForGroup_Options.db.profile.a_group_preferred_filters
						if legacy then
							return nil
						elseif legacy == false then
							return false
					...
					set = function(info,val)
						if val then
							LookingForGroup_Options.db.profile.a_group_preferred_filters = nil
						elseif val == false then
							LookingForGroup_Options.db.profile.a_group_preferred_filters = false
						else
							LookingForGroup_Options.db.profile.a_group_preferred_filters = true
						end
					end,
					tristate = true,
				},]]
						...
						if activityID ~= 0 then
							...
							if quest_id == nil then
								...
								if profile.addon_meeting_stone then
									...
									local mode = profile.addon_meeting_stone_mode or 1
									local loot = profile.addon_meeting_stone_loot or 1
									if LookingForGroup_MS == nil then
										LookingForGroup_MS = LookingForGroup.GetAddon("LookingForGroup_MS")
									end

									local mode_tb = LookingForGroup_MS.mode_tb()
									local loot_tb = LookingForGroup_MS.loot_tb()
									local tag = table.concat{mode_tb[mode],"-",loot_tb[loot],"-",title == "" and activityname or title}
									title = table.concat{loot_tb[loot],"-",mode_tb[mode],"-",title == "" and activityname or title}
									...
									if profile.addon_meeting_stone_hack then
										dt = table_concat{details,t or "","(^1^Z^T^N1^S",version,"^t^N",profile.addon_meeting_stone_mode or 1,"^N",profile.addon_meeting_stone_loot or 1,"^T^t^N",ilvl,"^T^t^T^t^N",max_player_level,"^N",max_player_level,req,"^Z^S",player,'-',realm,"^Z^S",tag,"^Z^^)"}
									else
										dt = table_concat{details,"(^1^Z^S",version,"^N",profile.addon_meeting_stone_mode or 1,"^N",profile.addon_meeting_stone_loot or 1,"^N",profile.addon_meeting_stone_class or select(3,UnitClass("player")),"^N",ilvl,"^N2147483647^",LookingForGroup_Options.ms_rating(activityID,profile),"^N",max_player_level,"^N",max_player_level,req,t and "^S"..t or "^Z","^S",player,'-',realm,"^Z^S",tag,"^Z^^)"}
									end
								...
								elseif title ~= "" then
									funcListing(activityID,
											...
											profile.start_a_group_minimum_honor_level,
											profile.start_a_group_voice_chat,
											t and table_concat{profile.start_a_group_details,"[",t,"]"} or profile.start_a_group_details,
											profile.start_a_group_auto_accept,
											profile.start_a_group_private)
									LookingForGroup_Options.set_requests()
									AceConfigDialog:SelectGroup("LookingForGroup","requests")
