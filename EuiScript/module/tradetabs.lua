...
local function UpdateTab(object, name, rank, texture)
	...
	if E.Skins then
		...
		tab:Point("TOPLEFT", object, "TOPRIGHT", 0, (-44 * index) + 18)
		tab:SetNormalTexture(texture)
	end
...
local function GetProfessionRank(currentSkill)
	...
	for index = 1, #ranks do
		...
		if (currentSkill == requiredSkill + 10) or (currentSkill == requiredSkill + 15) then
			return title
		end
		if (index == #ranks) and (currentSkill > requiredSkill) then
			return title
		end
	end
