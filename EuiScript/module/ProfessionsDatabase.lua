...
--	local link = format("trade:%d:%d:%d:%s:%s", spellId, maxed, maxed, guid, suffix)
	local link = format("trade:%s:%s:333", guid, spellId)
...
local function DropDown_Init()
	for i, spellIds in ipairs(PROFESSIONS) do
		L_UIDropDownMenu_AddButton({isTitle = 1, notCheckable = 1, text = i == 1 and PRIMARY or SECONDARY})
...
local dropdown = CreateFrame("Frame", "ProfessionsDatabaseDropDown", UIParent, "L_UIDropDownMenuTemplate")
L_UIDropDownMenu_Initialize(dropdown, DropDown_Init, "MENU")
