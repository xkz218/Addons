...
local lodFrames = {
  ...
  Blizzard_ArtifactUI = {
	['ArtifactRelicForgeFrame'] = false,
	['ArtifactFrameUnderlay'] = false,
	['ArtifactFrame'] = false,
},
...
function addon:HookFrame(name, moveParent)
  ...
  if frame and not hooked[name] then
	...
	if name=="ArtifactRelicForgeFrame" then
		frame:SetFrameStrata("DIALOG") 
	end	

	if name == 'ArtifactFrame' then
		ArtifactFrame.PerksTab.TitleContainer:SetScript("OnMouseDown", function(farme, button)
			MouseDownHandler(ArtifactFrame, button);
		end)
		ArtifactFrame.PerksTab.TitleContainer:SetScript("OnMouseUp", function(farme, button)
			MouseUpHandler(ArtifactFrame, button);
		end)
	else
