...
function ProfilesDropDown_OnClick(sender)
    L_UIDropDownMenu_SetSelectedID(TalentProfiles_profilesDropDown, sender:GetID())
end
...
function BuildFrame()
    ...
    if TalentProfiles_main == nil then
        ...
        if isElvUILoaded then
            ...
            mainFrame:SetPoint("TOPLEFT", PlayerTalentFrameTopTileStreaks, "TOPLEFT", 45, -7)
        end
    end
    ...
    if TalentProfiles_profilesDropDown == nil then
        dropdown = CreateFrame("Button", "TalentProfiles_profilesDropDown", TalentProfiles_main, "L_UIDropDownMenuTemplate")
        if isElvUILoaded then
            ...
            L_UIDropDownMenu_SetWidth(dropdown, 125)
            L_UIDropDownMenu_SetButtonWidth(dropdown, 125)
            dropdown:SetPoint("TOPLEFT", TalentProfiles_main, "TOPLEFT", 0, 0)
        end
    end
    -- Repopulate the dropdown, even if it already exists
    L_UIDropDownMenu_Initialize(dropdown, ProfilesDropDown_Initialise)
    L_UIDropDownMenu_SetSelectedID(dropdown, 1)
