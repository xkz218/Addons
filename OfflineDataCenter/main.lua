...
local function ChooseChar_OnClick(self)
	...
	L_UIDropDownMenu_SetSelectedValue(ODC.Frame.chooseChar, self.value);
...
local function ChooseCharMenuInitialize(self, level)
	...
	L_UIDropDownMenu_SetSelectedValue(ODC.Frame.chooseChar, ODC.selectChar);
	OfflineDataCenterFrameChooseCharDropDownText:SetWidth(140)
end
...
local function CreateODCFrame()
	...
	f.chooseChar = CreateFrame('Frame', name..'ChooseCharDropDown', f, 'L_UIDropDownMenuTemplate')
	f.chooseChar:SetPoint("LEFT", f.title, f.title:GetStringWidth(), -5)
	L_UIDropDownMenu_Initialize(f.chooseChar, ChooseCharMenuInitialize)
	--L_UIDropDownMenu_SetWidth(OfflineDataCenterFrameChooseCharDropDown, 200);
...
local function AddPopMenu(tabName, tabTooltip)
	local t = {text = tabTooltip,
		func = function() PlaySound(850); ODC:SetActiveTab(tabName, true) end,}
...
StaticPopupDialogs["ODC_ACCEPT_WIPE_ALL"] = {
    ...
    OnAccept = function(self)
		...
		L_UIDropDownMenu_Initialize(ODC.Frame.chooseChar, ChooseCharMenuInitialize)
