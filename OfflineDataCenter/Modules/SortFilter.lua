...
local function Filter_OnClick(self)
	L_UIDropDownMenu_SetSelectedValue(ODCFrameSortFilterSubFrameFilterDropDown, self.value);
...
local function FilterMenuInitialize(self, level)
	...
	for keyword, t in pairs(subIdxTb) do
		...
		local t
		local method = L_UIDropDownMenu_GetSelectedValue(ODCFrameSortFilterSubFrameSortDropDown)
	...
	if L_UIDropDownMenu_GetSelectedValue(ODCFrameSortFilterSubFrameFilterDropDown) == nil or selectSortChanged == true then
		L_UIDropDownMenu_SetSelectedValue(ODCFrameSortFilterSubFrameFilterDropDown, "__all");
		selectSortChanged = nil
	end
end

local function SortMethod_OnClick(self)
	L_UIDropDownMenu_SetSelectedValue(ODCFrameSortFilterSubFrameSortDropDown, self.value);
	selectSortChanged = true
	Module:Update("sort")
	--local text = OfflineDataCenterFrameSortDropDownText;
	--local width = text:GetStringWidth();
	--L_UIDropDownMenu_SetWidth(OfflineDataCenterFrameSortDropDown, width+40);
...
local function SortMenuInitialize(self, level)
	...
	L_UIDropDownMenu_SetSelectedValue(ODCFrameSortFilterSubFrameSortDropDown, "No sorting");
	--local text = OfflineDataCenterFrameSortDropDownText;
	--local width = text:GetStringWidth();
	--L_UIDropDownMenu_SetWidth(OfflineDataCenterFrameSortDropDown, width+40);
	--OfflineDataCenterFrameSortDropDown:SetWidth(width+60)
...
local function Filter()
	...
	slotDB = nil
	sumQuality = {}
	local filter = L_UIDropDownMenu_GetSelectedValue(ODCFrameSortFilterSubFrameFilterDropDown)
...
local function SortDB()
	...
	selectSortChanged = true
	L_UIDropDownMenu_Initialize(ODCFrameSortFilterSubFrameFilterDropDown, FilterMenuInitialize)
	if not subIdxTb then return end
	Filter()
end
...
local function CreateSubFrame()
	...
	f.sortmethod = CreateFrame('Frame', FRAMENAME..'SortDropDown', f, 'L_UIDropDownMenuTemplate')
	f.sortmethod:SetPoint("BOTTOMLEFT", f.searchingBar, 0, -36)
	
	----Create filter dropdown menu
	f.filter = CreateFrame('Frame', FRAMENAME..'FilterDropDown', f, 'L_UIDropDownMenuTemplate')
...
local function UpdateSortMenu()
	L_UIDropDownMenu_Initialize(Module.Frame.sortmethod, SortMenuInitialize)
end
