...
do
	...
	function Core:GetOptions(Addon, SubGroup)
		...
		local Name, Info
		local _, EL = unpack(ElvUI)
		if SubGroup then
			Name = SubGroup
			Info = (L["Adjust the skin of all buttons registered to %s: %s."]):format(EL[Addon], EL[SubGroup])
		elseif Addon then
			Name = Addon
			Info = (L["Adjust the skin of all buttons registered to %s. This will overwrite any per-group settings."]):format(EL[Addon])
		else
			Name = L["Global"]
