...
C.numberstyle = false		-- 啟用數字樣式，如果想要條形的血條就關閉這項/number style, if you want a bar-style nameplates, change to false.
...
G.aurafontsize = 15																-- 光環字體大小/Aura font size
G.fontflag = "OUTLINE"															-- "OUTLINE" or none/描邊

-- [[ Config ]] --

C.Inset = true						-- 名條貼齊畫面邊緣/Let Nameplates don't go off screen
C.MaxDistance = 40 					-- 名條顯示的最大距離/max distance for nameplate show on
C.SelectedScale = 1.3					-- 縮放當前目標的名條大小/scale select target nameplate
C.MinAlpha = 0.9					-- 非當前目標與遠距離名條的透明度/set fadeout for out of range and non-target
...
C.auranum = 8						-- 圖示數量/the number of auras
C.auraiconsize = 24					-- 圖示大小/aura icon size
