...
function CD:UpdateResource(frame, active, coloring, partial)
  if active then
  ...
  -- frame:UpdatePartial(partial)
end

function CD:UpdatePartial(frame, textFrame, partial, residual)
  -- print("Testing", partial, residual)
  if partial then
    frame.statusbar:SetValue(residual)
    frame:Show()
    textFrame.fontString:SetText(tostring(residual))
    textFrame.fontString:Show()
    textFrame:Show()
  else
    frame:Hide()
    textFrame.fontString:SetText("0")
    textFrame.fontString:Hide()
    textFrame:Hide()
  end
end
...
function CD:UpdateDoomPrediction(position, indicator)
  if indicator then
    if textEnable and (SATimers[position] ~= nil) then
      local SATimer = SATimers[position]
      SATimer:SetTimer(indicator)
      SATimer:Show()
    end
    if statusbarEnable and (statusbars[position] ~= nil) then
...
function CD:Update()
  ...
  for i = 1, MAX_RESOURCE do
    if resourceEnable then
      local active = i <= math.floor(resource)
      local residual = (resource - (i-1)) * 10
      local partial = (residual < 10) and (residual > 0)
      -- print("wtf", partial, active, i, resource)
      self:UpdateResource(resourceFrames[i],
                          active,
                          i > spendThreshold and "spending" or (resourceCappedEnable and resource == MAX_RESOURCE) and "capped" or nil)
      if isDestruction then
        self:UpdatePartial(partialFrames[i], partialTextFrames[i], partial, residual)
      else
        self:UpdatePartial(partialFrames[i], partialTextFrames[i], false, 0)
      end
    end
  ...
  local t = 1
  local indicator = indicators[t]
  for i = math.floor(resource) + 1, statusbarCount do
...
local function buildFrames()
  ...
  if resourceEnable then
    ...
    local function createPartialFrame(numeration)
      local frame
      local statusbar
      if partialFrames[numeration] then
        frame = partialFrames[numeration]
        statusbar = frame.statusbar
      else
        frame = CreateFrame("Frame", "$parentPartial", CDFrame)
        statusbar = CreateFrame("StatusBar", "$parentPartialStatus", frame)
      ...
      if numeration <= MAX_RESOURCE then
        ...
        if not frame.border then
          frame.border = CreateFrame("frame", "$parentPartialBorder", frame)
        end
        frame.border:SetAllPoints(frame)
        frame.border:SetBackdrop(borderBackdrop)
        frame.border:SetBackdropBorderColor(db.borderColor.r, db.borderColor.b, db.borderColor.g, db.borderColor.a)
        ...
        statusbar:SetStatusBarColor(cr, cb, cg, ca)
    ...
    for i = 1, statusbarCount do
      partialFrames[i] = createPartialFrame(i)
    end

    function createPartialText(numeration)
      local parentFrame
      local fontString
      if not partialTextFrames[numeration] then
        parentFrame = partialTextFrames[numeration] or CreateFrame("frame", "$parentTextParent", CDFrame)
        parentFrame:SetFrameStrata("MEDIUM")
        -- parentFrame:Show()
        fontString = parentFrame:CreateFontString(nil, "OVERLAY")
        parentFrame.fontString = fontString
      else
        parentFrame = partialTextFrames[numeration]
      ...
      fontString:SetTextColor(c1r, c1b, c1g, c1a)
    ...
    for i=1, statusbarCount do
      partialTextFrames[i] = createPartialText(i)
    end
  else
    ...
    for i = 1, #partialFrames do
      partialFrames[i]:Hide()
    end
  end
...
function CD:Build()
  ...
  isDestruction = DS.specializationID == 267
