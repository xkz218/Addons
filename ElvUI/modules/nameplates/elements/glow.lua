...
function mod:UpdateElement_Glow(frame)
	...
	if (shouldShow ~= 0) then
		...
		if shouldShow ~= 2 and (self.db.targetGlow == "style3" or self.db.targetGlow == "style5" or self.db.targetGlow == "style6") then -- top arrow
			frame.TopArrow:ClearAllPoints();
			frame.TopArrow:SetPoint("BOTTOM", frame.HealthBar, "TOP", 0, self.db.targetGlow_y or E:Scale(E.Border*2))
			frame.TopArrow:Size(self.db.targetGlow_y_size or 45)
			frame.TopArrow:Show()
		end
		if shouldShow ~= 2 and (self.db.targetGlow == "style4" or self.db.targetGlow == "style7" or self.db.targetGlow == "style8") then -- side arrows
			frame.RightArrow:ClearAllPoints();
			frame.LeftArrow:ClearAllPoints();
			frame.RightArrow:SetPoint("RIGHT", frame.HealthBar, "LEFT", self.db.targetGlow_x or E:Scale(E.Border*2), 0)
			frame.LeftArrow:SetPoint("LEFT", frame.HealthBar, "RIGHT", -(self.db.targetGlow_x or E:Scale(E.Border*2)), 0)
			frame.LeftArrow:Size(self.db.targetGlow_x_size or 45)
			frame.RightArrow:Size(self.db.targetGlow_x_size or 45)
...
function mod:ConstructElement_Glow(frame)
	...
	top:Size(self.db.targetGlow_y_size or 45)
	top:Hide()
	frame.TopArrow = top
	...
	left:Size(self.db.targetGlow_x_size or 45)
	left:Hide()
	...
	right:Size(self.db.targetGlow_x_size or 45)
	right:Hide()
