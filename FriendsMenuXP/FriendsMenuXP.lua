...
function FriendsMenuXP_OnLoad(self)
    ...
    SetOrHookScript(DropDownList1, "OnHide", function()
        FriendsMenuXP:Hide();
        if(not InCombatLockdown()) then FriendsMenuXPSecure:Hide(); end
        L_UIDropDownMenu_StopCounting(FriendsMenuXP)
        L_UIDropDownMenu_StopCounting(FriendsMenuXPSecure)
    end)
...
--===============================================================================================
--The following codes are based on Blizzard's FrameXMLs, includes UnitPopup and L_UIDropDownMenu.
...
function FriendsMenuXP_Show(listFrame, arg2, appendBottom, name, connected, lineID, chatType, chatFrame, friendsList, isMobile)
    ...
    if(not relativeFrame) then PlaySound(88); end --open at last place should not play sound.
...
hooksecurefunc("L_UIDropDownMenu_StartCounting", function(self)
    if self==DropDownList1 then
        if (FriendsMenuXP:IsVisible()) then
            L_UIDropDownMenu_StartCounting(FriendsMenuXP)
        elseif (FriendsMenuXPSecure:IsVisible()) then
            L_UIDropDownMenu_StartCounting(FriendsMenuXPSecure)
        end
    end
end)

hooksecurefunc("L_UIDropDownMenu_StopCounting", function(self)
    if self==DropDownList1 then
        if (FriendsMenuXP:IsVisible()) then
            L_UIDropDownMenu_StopCounting(FriendsMenuXP)
        elseif (FriendsMenuXPSecure:IsVisible()) then
