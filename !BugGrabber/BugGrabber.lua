...
-- API
--
local function SaveArryToStr(a)
	local tmp = ''
	a.message = a.message or ''
	a.stack = a.stack or ''
	a.locals = a.locals or ''
	a.session = a.session or ''
	a.time = a.time or ''
	tmp = a.message..a.stack..a.locals..a.session..a.time
...
function addon:StoreError(errorObject)
	if db then
		...
		if #ElvUI[1].global.luaError > 21 then wipe(ElvUI[1].global.luaError) end
		
		if not ElvUI[1].global.luaError then ElvUI[1].global.luaError = {} end
		ElvUI[1].global.luaError[#ElvUI[1].global.luaError + 1] = SaveArryToStr(errorObject)
		...
		if #ElvUI[1].global.luaError > 20 then
			table.remove(ElvUI[1].global.luaError, 1)
		end
	else
...
function addon:Reset() if BugGrabberDB then wipe(BugGrabberDB.errors); wipe(ElvUI[1].global.luaError); end end
