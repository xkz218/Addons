...
function LookingForGroup_MS.mode_tb()
	return {"开荒","带新","成就","幻化","冲分","混分","荣誉","练习","任务","日常","练级","娱乐","碾压","世界",[0]=ALL}
end

function LookingForGroup_MS.loot_tb()
	return {"自由拾取","轮流拾取","队长分配","队伍分配","需求优先","个人拾取",[0]=ALL}
end

function LookingForGroup_MS.hsr(data)
	wipe(concat_tb)
    local isMeetingStone, customId, version, mode, loot,
            class, itemLevel, progression, leaderPvPRating, minLevel, maxLevel, pvpRating, source, creator, savedInstance, check, honorLevel = LookingForGroup_MS:Deserialize(data)
	warning()
